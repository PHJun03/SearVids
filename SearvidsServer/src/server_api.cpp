...
const std::string DB_CONN_STR =
    "postgresql://searvids_user:searvids_pass@db:5432/searvids_db";
...
void init_db() {
  try {
    ...
    if (C.is_open()) {
      ...
      try {
        W.exec("ALTER TABLE videos ADD COLUMN IF NOT EXISTS access_count INT "
               "DEFAULT 1;");
      } catch (...) {
      }
      try {
        W.exec("ALTER TABLE videos ADD COLUMN IF NOT EXISTS last_accessed_at "
               "TIMESTAMP DEFAULT CURRENT_TIMESTAMP;");
      } catch (...) {
      }
      try {
        W.exec(
            "ALTER TABLE videos ADD COLUMN IF NOT EXISTS duration_ms BIGINT;");
      } catch (...) {
      }
...
void enforce_disk_cache_policy() {
  ...
  try {
    ...
    if (C.is_open()) {
      ...
      W.exec("DELETE FROM access_logs WHERE accessed_at < NOW() - INTERVAL '30 "
             "days'");

      // 2. Find videos to evict based on MONTHLY popularity (count in
      // access_logs) We select videos that have index_path, order by their log
      // count DESC, and skip top N.
      ...
      for (const auto &row : R) {
        ...
        try {
          if (std::filesystem::exists(path + ".index"))
            std::filesystem::remove(path + ".index");
          if (std::filesystem::exists(path + ".meta"))
            std::filesystem::remove(path + ".meta");
          std::cout << "[Cache Eviction] Removed " << vid
                    << " (Low Monthly Rank)" << std::endl;
        } catch (...) {
        }

        // Update DB
        W.exec_params("UPDATE videos SET index_path = NULL WHERE video_id = $1",
                      vid);
      }
      W.commit();
    }
...
void broadcast_status(const std::string &video_id, const VideoSession &sess) {
  ...
  j["status"] =
      sess.error.empty()
          ? (sess.done ? "done" : (sess.analyzing ? "analyzing" : "pending"))
          : "error";
  ...
  j["indexed_audio_segments"] =
      static_cast<int64_t>(sess.indexed_audio_segments);
...
static siglip_onnx::SiglipOnnx &get_siglip() {
  ...
  if (!g_siglip_ptr) {
    ...
    if (const char *env_p = std::getenv("USE_GPU")) {
      ...
      if (env_s == "true" || env_s == "1")
        use_gpu = true;
    }
    std::cout << "[Server] Initializing SigLIP with GPU="
              << (use_gpu ? "ON" : "OFF") << std::endl;
...
void analyze_video_async(std::shared_ptr<VideoSession> sess_ptr) {
  ...
  try {
    ...
    if (!is_local) {
      std::cout << "[" << sess.video_id << "] Getting video duration..."
                << std::endl;
    ...
    if (total_duration_sec > 0) {
      ...
      std::cout << "[" << sess.video_id
                << "] Chunk-based analysis. Total duration: "
                << total_duration_sec << "s" << std::endl;
      ...
      while (current_start < total_duration_sec) {
        ...
        int current_end =
            std::min((int)total_duration_sec, current_start + CHUNK_SIZE);

        // Update stage
        sess.current_stage = "processing_chunk_" +
                             std::to_string(current_start) + "_" +
                             std::to_string(current_end);
        broadcast_status(sess.video_id, sess);

        // Download Chunk (Blocking, but runs in parallel with
        // last_analysis_future)
        std::string chunk_filename =
            sess.video_id + "_chunk_" + std::to_string(current_start) + ".mp4";
        std::string chunk_path = "data/" + chunk_filename;

        std::cout << "[" << sess.video_id << "] Downloading chunk "
                  << current_start << "-" << current_end << "s" << std::endl;
        if (!downloader::download_section(sess.source_url, chunk_path,
                                          current_start, current_end)) {
          if (current_start == 0)
            throw std::runtime_error("Failed to download first chunk");
          std::cerr << "Failed to download chunk " << current_start << "-"
                    << current_end << ", stopping." << std::endl;
          break;
        }
        ...
        last_analysis_future = std::async(std::launch::async, [=, &sess]() {
          try {
            ...
            auto visual_future = std::async(
                std::launch::async, [&sess, &info, chunk_path, current_start,
                                     total_duration_sec]() {
                  if (!info.has_video)
                    return;
                  try {
                    ...
                    auto consumer_thread = std::thread([&sess, &siglip,
                                                        &frame_queue,
                                                        current_start,
                                                        total_duration_sec]() {
                      while (true) {
                        ...
                        for (size_t i = 0; i < items.size(); ++i) {
                          if (items[i].is_end) {
                            received_end = true;
                            // Don't break immediately, process what we have
                            // first? Actually if is_end is in batch, subsequent
                            // items shouldn't exist ideally. But let's just
                            // stop collecting.
                            break;
                          }
                          batch_rgb.push_back(
                              std::move(items[i].frame.rgb_data));
                          ...
                          try {
                            ...
                            for (size_t k = 0; k < results.size(); ++k) {
                              ...
                              int64_t real_timestamp_ms =
                                  frame.timestamp_ms + (current_start * 1000);

                              hnsw_index::add(
                                  emb, sess.video_id,
                                  static_cast<float>(real_timestamp_ms) /
                                      1000.0f,
                                  static_cast<float>(real_timestamp_ms) /
                                          1000.0f +
                                      1.0f,
                              ...
                              if (k == results.size() - 1) {
                                double progress =
                                    (double)real_timestamp_ms /
                                    (double)(total_duration_sec * 1000);
                                if (progress > 1.0)
                                  progress = 1.0;
                                int p = (int)(progress * 100.0);
                                if (p > sess.progress_percent)
                                  sess.progress_percent = p;
                                broadcast_status(sess.video_id, sess);
                              }
                            }
                          } catch (...) {
                    ...
                    ffmpeg_decoder::extract_frames_with_callback(
                        ...
                        2.0, 0, 0, 0, 0, 0,
                        ffmpeg_decoder::FrameExtractionMethod::SCENE_DETECT,
                        10.0 // Adaptive: Force split every 10s if no scene
                             // change
                    );
            ...
            auto audio_future = std::async(
                std::launch::async, [&sess, &info, chunk_path, current_start,
                  ...
                  try {
                    ...
                    if (!ffmpeg_decoder::extract_audio(chunk_path, audio_path,
                                                       16000, 1, nullptr))
                      return;
                    ...
                    g_whisper.transcribe_with_callback(
                        audio_path,
                        [&sess, &siglip, &re, &buffer, current_start,
                         total_duration_sec](const std::string &chunk) {
                          if (sess.cancelled)
                            return;
                          buffer += chunk;
                          size_t pos;
                          while ((pos = buffer.find('\n')) !=
                            ...
                            if (std::regex_search(line, match, re)) {
                              try {
                                float start = std::stof(match[1]) * 3600 +
                                              std::stof(match[2]) * 60 +
                                              std::stof(match[3]) +
                                              std::stof(match[4]) / 1000.0f;
                                float end = std::stof(match[5]) * 3600 +
                                            std::stof(match[6]) * 60 +
                                            std::stof(match[7]) +
                                            std::stof(match[8]) / 1000.0f;
                                ...
                                if (!text.empty()) {
                                  ...
                                  hnsw_index::add(emb, sess.video_id, start,
                                                  end, text);
            ...
            std::cerr << "Error analyzing chunk " << current_start << ": "
                      << e.what() << std::endl;
          }
        });
      ...
      if (!is_local) {
        ...
        std::cout << "[" << sess.video_id
                  << "] Downloading: " << sess.source_url << std::endl;

        bool ok = downloader::download_with_retry(sess.source_url, out, 3);
        if (!ok)
          throw std::runtime_error("Download failed after 3 retries");

        sess.local_path = out;
        sess.progress_percent = 10;
        std::cout << "[" << sess.video_id << "] Download complete: " << out
                  << std::endl;
      } else {
        // Local file, path is already set
        std::cout << "[" << sess.video_id
                  << "] Using local file: " << sess.local_path << std::endl;
        sess.progress_percent = 10;
      }

      // 2) Probe video metadata (10-15%)
      sess.current_stage = "probing";
      std::cout << "[" << sess.video_id << "] Probing video metadata..."
                << std::endl;
      ...
      sess.nb_frames = (info.nb_frames > 0)
                           ? info.nb_frames
                           : ffmpeg_decoder::count_frames(sess.local_path);
      ...
      auto visual_future = std::async(std::launch::async, [&sess, &info]() {
        if (!info.has_video)
          return;
        try {
          std::cout << "[" << sess.video_id << "] Extracting visual frames..."
                    << std::endl;
          auto &siglip = get_siglip();
                ...
                try {
                  ...
                  for (size_t k = 0; k < results.size(); ++k) {
                    ...
                    if (sess.nb_frames > 0 && k == results.size() - 1) {
                      double progress =
                          (double)frame.timestamp_ms / (double)sess.duration_ms;
                      if (progress > 1.0)
                        progress = 1.0;
                  ...
                  std::cerr << "[" << sess.video_id
                            << "] CLIP inference failed: " << e.what()
                            << std::endl;
                }
              }
          ...
          ffmpeg_decoder::extract_frames_with_callback(
              ...
              2.0, 0, 0, 0, 224, 224,
              ffmpeg_decoder::FrameExtractionMethod::SCENE_DETECT,
              10.0 // Adaptive: Force split every 10s if no scene change
          );
          ...
          std::cout << "[" << sess.video_id << "] Indexed "
                    << sess.indexed_visual_frames << " visual frames"
                    << std::endl;
        } catch (const std::exception &e) {
          std::cerr << "[" << sess.video_id
                    << "] Visual processing failed: " << e.what() << std::endl;
        }
      });
      ...
      auto audio_future = std::async(std::launch::async, [&sess, &info]() {
        ...
        try {
          ...
          if (!ffmpeg_decoder::extract_audio(sess.local_path, audio_path, 16000,
                                             1, nullptr)) {
            throw std::runtime_error("Failed to extract audio");
          }

          std::cout << "[" << sess.video_id
                    << "] Starting transcription (CTranslate2)..." << std::endl;
                  ...
                  if (std::regex_search(line, match, re)) {
                    try {
                      ...
                      if (!text.empty()) {
                        ...
                        // Update progress (Audio is approx 45% of total, from
                        // 45% to 90%) We use end time to estimate progress
                        if (sess.duration_ms > 0) {
                          double progress =
                              (double)(end * 1000.0) / (double)sess.duration_ms;
                          if (progress > 1.0)
                            progress = 1.0;
                          int p =
                              15 + 30 + (int)(45.0 * progress); // 45% to 90%
                          if (p > sess.progress_percent)
                            sess.progress_percent = p;
                        }
          ...
          std::filesystem::remove(audio_path);
          std::cout << "[" << sess.video_id << "] Indexed "
                    << sess.indexed_audio_segments << " audio segments"
                    << std::endl;

        } catch (const std::exception &e) {
          std::cerr << "[" << sess.video_id
                    << "] Audio processing failed: " << e.what() << std::endl;
        }
      });
    ...
    std::cout << "[" << sess.video_id << "] Analysis completed in " << duration
              << " seconds (audio_segments: " << sess.indexed_audio_segments
              << ", visual_frames: " << sess.indexed_visual_frames << ")"
              << std::endl;
    ...
    try {
      ...
      if (C.is_open()) {
        ...
        W.commit();
        std::cout << "[" << sess.video_id << "] Saved to DB cache."
                  << std::endl;
    ...
    if (!sess.local_path.empty() && sess.source_url.find("http") == 0 &&
        std::filesystem::exists(sess.local_path)) {
      std::filesystem::remove(sess.local_path);
      std::cout << "[" << sess.video_id << "] Removed temporary video file."
                << std::endl;
    }
    ...
    std::cerr << "[" << sess.video_id << "] Analysis failed: " << e.what()
              << std::endl;
...
SearchRequest parse_search_request(const std::string &body) {
  ...
  try {
    ...
    if (j.contains("query"))
      req.query = j["query"];
    if (j.contains("query_text"))
      req.query = j["query_text"];
    ...
    if (j.contains("offset"))
      req.offset = j["offset"];
    if (j.contains("min_similarity"))
      req.min_similarity = j["min_similarity"];
    if (j.contains("search_type"))
      req.search_type = j["search_type"];
    if (j.contains("video_id"))
      req.video_id = j["video_id"];
  } catch (...) {
  }
  return req;
}
...
crow::response
create_search_response(const std::vector<SearchResult> &results) {
  nlohmann::json j_results = nlohmann::json::array();
  for (const auto &r : results) {
    nlohmann::json item;
    item["id"] = r.id;
    item["video_id"] = r.video_id; // Added video_id to JSON
    item["start_time"] = r.start_time;
    item["end_time"] = r.end_time;
    item["caption"] = r.caption;
    item["score"] = r.similarity;
    item["type"] = r.result_type.empty()
                       ? (r.caption == "visual_frame" ? "visual" : "audio")
                       : r.result_type;
...
filter_results(const std::vector<hnsw_index::TimelineEntry> &raw_results,
               const std::string &query, float min_score = -2.0f) {
  ...
  for (const auto &r : raw_results) {
    ...
    if (!is_visual && !query.empty()) {
      auto it = std::search(
          r.caption.begin(), r.caption.end(), query.begin(), query.end(),
    ...
    if (!keep) {
      float threshold =
          (min_score > -2.0f) ? min_score : (is_visual ? 0.01f : 0.15f);
    ...
    if (keep) {
      filtered.push_back({r.id, r.video_id, r.start_time, r.end_time, r.caption,
                          r.similarity});
    }
  }
  return filtered;
...
void setup_routes(crow::SimpleApp &app) {
  ...
  CROW_WEBSOCKET_ROUTE(app, "/api/ws/videos/<string>/status")
      .onopen([&](crow::websocket::connection &conn) {
        // Crow doesn't easily pass route args to onopen, so we wait for a
        // "subscribe" message.
      })
      .onmessage([&](crow::websocket::connection &conn, const std::string &data,
        ...
        try {
          ...
          if (j.contains("type") && j["type"] == "subscribe" &&
              j.contains("video_id")) {
            ...
            if (it != g_sessions.end()) {
              ...
              resp["status"] =
                  sess.error.empty()
                      ? (sess.done ? "done"
                                   : (sess.analyzing ? "analyzing" : "pending"))
                      : "error";
              resp["progress_percent"] =
                  static_cast<int>(sess.progress_percent);
              resp["indexed_visual_frames"] =
                  static_cast<int64_t>(sess.indexed_visual_frames);
              resp["indexed_audio_segments"] =
                  static_cast<int64_t>(sess.indexed_audio_segments);
        ...
        try {
          ...
          if (C.is_open()) {
            ...
            pqxx::result R = W.exec_params(
                "SELECT index_path FROM videos WHERE url = $1", url);
            if (!R.empty()) {
              std::string index_path = R[0][0].as<std::string>();
              if (std::filesystem::exists(index_path + ".index")) {
                std::cout << "[Cache Hit] Loading index for " << vid
                          << std::endl;
      ...
      .methods(crow::HTTPMethod::POST)([](const crow::request &req) {
        ...
        try {
          ...
          if (C.is_open()) {
            pqxx::work W(C);
            pqxx::result R = W.exec_params(
                "SELECT index_path, duration_ms FROM videos WHERE url = $1",
                url);
            if (!R.empty()) {
              std::string index_path = R[0][0].as<std::string>();
  ...
  CROW_ROUTE(app, "/videos/<string>/status")
      .methods(crow::HTTPMethod::GET)([](const std::string &video_id) {
        std::lock_guard<std::mutex> lk(g_sessions_mtx);
        ...
        j["indexed_visual_frames"] =
            static_cast<int64_t>(s.indexed_visual_frames);
        j["indexed_audio_segments"] =
            static_cast<int64_t>(s.indexed_audio_segments);
        return json_ok(j);
      });

  // Alias for proxy: GET /api/videos/{video_id}/status
  CROW_ROUTE(app, "/api/videos/<string>/status")
      .methods(crow::HTTPMethod::GET)([](const std::string &video_id) {
        std::lock_guard<std::mutex> lk(g_sessions_mtx);
        auto it = g_sessions.find(video_id);
        if (it == g_sessions.end())
  ...
  CROW_ROUTE(app, "/videos/<string>/thumbnail")
      .methods(crow::HTTPMethod::GET)([](const crow::request &req,
        ...
        if (!ts_str)
          return crow::response(400, "timestamp required");
        ...
        if (path.empty() || !std::filesystem::exists(path)) {
          ...
          std::string chunk_path = "data/" + video_id + "_chunk_" +
                                   std::to_string(chunk_start) + ".mp4";
        ...
        try {
          ...
          auto frame =
              ffmpeg_decoder::extract_frame_at(path, actual_ts, true, 256, 144);
  ...
  CROW_ROUTE(app, "/api/videos/<string>/thumbnail")
      .methods(crow::HTTPMethod::GET)([](const crow::request &req,
                                         const std::string &video_id) {
        std::string path;
        {
  ...
  CROW_ROUTE(app, "/search")
      .methods(crow::HTTPMethod::POST)([](const crow::request &req) {
        ...
        std::cout << "[DEBUG] Search Query: "
                  << (sreq.query.empty() ? "(embedding)" : sreq.query)
                  << std::endl;
        std::cout << "[DEBUG] HNSW provided " << hnsw_results.size()
                  << " raw results." << std::endl;
        if (!hnsw_results.empty()) {
          std::cout << "[DEBUG] Top Result: " << hnsw_results[0].video_id
                    << " Score: " << hnsw_results[0].similarity << std::endl;
        }

        // 3) filter and convert
        auto api_results =
            filter_results(hnsw_results, sreq.query, sreq.min_similarity);
        std::cout << "[DEBUG] After filtering: " << api_results.size()
                  << " results." << std::endl;
        ...
        try {
          ...
          if (sreq.embedding.empty()) {
            try {
              ...
              return json_err(500, std::string("SigLIP encoding failed: ") +
                                       e.what());
            }
          } else {
            emb = sreq.embedding;
          ...
          if (sreq.search_type == "visual") {
            hnsw_results =
                hnsw_index::search(emb, sreq.topk, sreq.video_id, "visual");
          } else if (sreq.search_type == "audio") {
            hnsw_results =
                hnsw_index::search(emb, sreq.topk, sreq.video_id, "audio");
          } else {
            // Hybrid Fusion Logic
            // We combine results from both modalities.
            // Strategy: Weighted sum if both exist at similar timestamp, or
            // keep individual with weight. Weights: Visual 0.6, Audio 0.4
            // (heuristics)

            auto visual_results =
                hnsw_index::search(emb, sreq.topk * 2, sreq.video_id, "visual");
            auto audio_results =
                hnsw_index::search(emb, sreq.topk * 2, sreq.video_id, "audio");

            // Map key: {video_id, timestamp} -> {combined_score, entry_data}
            // We use a simple normalization: assume raw cosine similarity is
            // roughly comparable
            std::map<std::pair<std::string, int64_t>, hnsw_index::TimelineEntry>
                fused_map;

            const float w_vis = 0.6f;
            const float w_aud = 0.4f;

            for (const auto &r : visual_results) {
              std::pair<std::string, int64_t> key(
                  r.video_id, (int64_t)(r.start_time * 1000));
              fused_map[key] = r;
              fused_map[key].similarity = r.similarity * w_vis;
            }

            for (const auto &r : audio_results) {
              std::pair<std::string, int64_t> key(
                  r.video_id, (int64_t)(r.start_time * 1000));
              if (fused_map.count(key)) {
                // Fusion: Both modalities found this segment!
                // Additive boost (You might want Max or Avg, but Additive
                // rewards multimodal matches)
                fused_map[key].similarity += (r.similarity * w_aud);
              } else {
                fused_map[key] = r;
                fused_map[key].similarity = r.similarity * w_aud;
              }
            ...
            for (const auto &kv : fused_map) {
              hnsw_results.push_back(kv.second);
            }
  ...
  CROW_ROUTE(app, "/api/upload")
      .methods(crow::HTTPMethod::POST)([](const crow::request &req) {
        ...
        if (content.empty())
          return json_err(400, "No file uploaded");
  ...
  CROW_ROUTE(app, "/api/videos/<string>")
      .methods(crow::HTTPMethod::DELETE)([](const std::string &video_id) {
        std::lock_guard<std::mutex> lk(g_sessions_mtx);
        auto it = g_sessions.find(video_id);
        if (it != g_sessions.end()) {
          it->second->cancelled = true;
          if (!it->second->local_path.empty() &&
              std::filesystem::exists(it->second->local_path)) {
        ...
        for (const auto &entry : std::filesystem::directory_iterator("data")) {
          if (entry.path().filename().string().find(video_id) !=
              std::string::npos) {
  ...
  CROW_ROUTE(app, "/api/debug/text_sim")
      .methods(crow::HTTPMethod::POST)([](const crow::request &req) {
